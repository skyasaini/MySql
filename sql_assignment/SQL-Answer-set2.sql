-- Active: 1683892889505@@127.0.0.1@3306
...
select employee_id,tmp.team_size
from Employee E left join 
(select team_id, count(team_id) as team_size from Employee group by team_id) as tmp
...
select A.player_id, A.device_id from Activity A right join 
(select player_id,Min(event_date) as d from Activity GROUP BY player_id) as tmp
on tmp.d = A.event_date;
------------------or---------
...
--Return the result table in any order.
with seller_wise_sales as (select seller_id, sum(price) as t_price from Sales GROUP BY seller_id)

select seller_id from seller_wise_sales
where t_price = (select max(t_price) from seller_wise_sales);
...
with groupby_visited as (select visited_on, sum(amount) as amount from Customer GROUP BY visited_on)
...
from groupby_visited) as outputtable where visited_on > '2019-01-06';
